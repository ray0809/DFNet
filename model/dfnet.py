...
from .modules import resize_like
...
class DecodeBlock(nn.Module):
    ...
    def forward(self, x, concat):
        size = concat.shape[2:]
        out = F.interpolate(x, size, mode=self.mode)
...
class DFNet(nn.Module):
    def __init__(
            ...
            blend_layers=[0, 1, 2, 3, 4, 5], freeze_en_bn=False):
        super().__init__()
        self.freeze_en_bn = freeze_en_bn
        c_init = c_img + c_mask
    ...
    def train(self, mode=True):
        ...
        """
        super().train(mode)
        if not self.freeze_en_bn:
            return 
        for name, module in self.named_modules():
            if isinstance(module, nn.BatchNorm2d) and 'encode' in name:
                module.eval()
